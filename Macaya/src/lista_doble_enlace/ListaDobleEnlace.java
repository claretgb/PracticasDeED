...
public class ListaDobleEnlace<E> implements ILista<E> {
	...
	public ILista<E> extrae(int a, int b) {
		ListaDobleEnlace<E> listaDevuelve = new ListaDobleEnlace<E>();
		Celda<E> aux = principio.siguiente;
		for(int i = 0; i < a; i++) {
			aux = aux.siguiente;
		}
		if(a >= 0 && b < numEle && a < b) {
			for(int i = 1; i <= b - a + 1; i++) {
				Celda<E> auxiliar = aux;
				aux.anterior.siguiente = aux.siguiente;
				aux.siguiente.anterior = aux.anterior;
				aux = aux.siguiente;
				numEle--;
				auxiliar.anterior = listaDevuelve.fin.anterior;
				auxiliar.siguiente = listaDevuelve.fin;
				listaDevuelve.fin.anterior = auxiliar;
				auxiliar.anterior.siguiente = auxiliar;
				listaDevuelve.numEle++;
			}
		}
